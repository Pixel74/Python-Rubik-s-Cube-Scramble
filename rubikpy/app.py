...
def upd_cube_img():
    g_cube_img = ImageTk.PhotoImage(Image.open(os.path.join('..', 'output', '-----Final-----.jpg')))
...
reset_img_btn = Button(root, text='Reset Cube', command=reset, bg='#00ff4c', fg='#000000', activebackground='#29c40e',
                       height=2, width=15, font='Helveltica')
reset_img_btn.grid(column=5, row=2, columnspan=2)

# Buttons and selectors Section

# Length selector
len_scramble_sp = Spinbox(root, from_=0, to=300)
len_scramble_sp.grid(column=5, row=5, columnspan=2, sticky=N)


# Auto-scrambler generator
def new_scramble():
    str_scramble = scramble_generator(int(len_scramble_sp.get()))
    make_scramble(str_scramble, print_moves=False, gen_img=True)

    text_box.delete('1.0', END)
    text_box.insert(END, str_scramble)
...
# Insert a scramble
def get_scramble():
    str_scramble = str(scramble_text.get())
    make_scramble(str_scramble)
...
# Clock Buttons
reset_time_btn = Button(root, text='Reset Time', command=start_clock, bg='#f6ff00', fg='#000000',
                        activebackground='red', height=2, width=10, font='Helveltica')
reset_time_btn.grid(column=5, row=1)

stop_time_btn = Button(root, text='Stop Time', command=stop_clock, bg='#f6ff00', fg='#000000',
                       activebackground='#f7c10f', height=2, width=10, font='Helveltica')
